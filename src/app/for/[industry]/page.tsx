...
const industries: Record<string, IndustryData> = {
  ...
  ec: {
    ...
    intro: [
      ...
      "特に重要なのは、AIクローラーがサイトにアクセスできる状態を作ることです。robots.txtでGPTBotやClaudeBotをブロックしているECサイトは多く、それだけで機会損失が発生しています。まずは現状のGEOスコアを確認し、改善ポイントを特定することから始めましょう。",
    ],
    challenges: [
      {
...
Sitemap: https://example.com/sitemap.xml`,
  ...
  saas: {
    ...
    intro: [
      "「おすすめのプロジェクト管理ツール」「CRM 比較 2026」---このようなクエリに対してAIが直接ツールを推薦する時代において、SaaSプロバイダにとってGEO対策はリード獲得の新しいチャネルです。従来のリスティング広告やコンテンツSEOに加え、AIの推薦リストに載ることが成長戦略の柱になりつつあります。",
...
- 相続税申告: 遺産総額の0.5%〜`,
  ...
  local: {
    ...
    intro: [
      "「渋谷 おすすめ イタリアン」「新宿 肩こり 整体」---地域名を含む検索クエリは、AI検索でも非常に多いパターンです。ChatGPTやPerplexityは、ユーザーの質問に対して具体的な店舗名とその特徴を回答するようになっており、ここで推薦されるかどうかが来店数に直結します。",
      ...
      "特に飲食店や美容院では、メニュー情報と口コミ・評価の構造化が最も効果的です。AIは「雰囲気が良い」「コスパが高い」といった定性的な情報よりも、「ランチ 1,200円〜」「平均評価4.5（口コミ300件）」のような定量情報を好んで引用します。",
    ],
    challenges: [
      {
        title: "店舗情報がGBPにしかない",
