...
function generateMarkdownCompare(results: { url: string; report: CheckReport | null; error: string }[]): string {
  const reports = results.filter((r) => r.report !== null);
  if (reports.length === 0) return "";

  const indicators = reports[0].report!.results.map((r) => r.message.split(":")[0]);
  const lines: string[] = [];
  lines.push("# GEOスコア比較レポート");
  lines.push("");
  lines.push(`比較日時: ${new Date().toLocaleString("ja-JP")}`);
  lines.push("");

  // Summary table
  lines.push("## サマリー");
  lines.push("");
  lines.push("| URL | グレード | スコア |");
  lines.push("|-----|---------|--------|");
  for (const r of reports) {
    const rpt = r.report!;
    const pct = Math.round((rpt.totalScore / rpt.maxScore) * 100);
    lines.push(`| ${rpt.url} | ${rpt.grade} | ${pct}/100 |`);
  }
  lines.push("");
  ...
  lines.push("## 指標別スコア");
  lines.push("");
  const headerRow = ["指標", ...reports.map((r) => { try { return new URL(r.report!.url).hostname; } catch { return r.report!.url; } })];
  lines.push(`| ${headerRow.join(" | ")} |`);
  lines.push(`| ${headerRow.map(() => "---").join(" | ")} |`);
  for (let i = 0; i < indicators.length; i++) {
    const cells = [indicators[i], ...reports.map((r) => {
      const item = r.report!.results[i];
      const icon = item.status === "pass" ? "o" : item.status === "warn" ? "△" : "x";
      return `${icon} ${item.score}/${item.maxScore}`;
    })];
...
export function CompareClient() {
  ...
  const handleExportMarkdown = useCallback(() => {
    const md = generateMarkdownCompare(results);
    if (!md) {
      setCsvError("エクスポートするデータがありません。チェック成功した結果が2件以上必要です。");
      setTimeout(() => setCsvError(""), 4000);
      return;
    ...
    const a = document.createElement("a");
    a.href = blobUrl;
    a.download = `geo-score-compare-${new Date().toISOString().split("T")[0]}.md`;
    a.click();
    URL.revokeObjectURL(blobUrl);
  }, [results]);
